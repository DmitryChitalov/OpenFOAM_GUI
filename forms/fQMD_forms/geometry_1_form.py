	...
	def out_frame_func(int_lng, prj_path, mesh_name_txt, pd_2, geometry_visible): 
		...
		if os.path.exists(initial_path_file):
			...
			while i <= obj['g']:
				...
				if int_lng == 'Russian':
					geometry_type_l = ["Три-поверхность", "Распределенная три-поверхность", "Закрытая три-поверхность", "Базовая фигура", "Набор базовых фигур"]
				elif int_lng == 'English':
				...
				if geometry_1_obj != None:
					if geometry_1_obj[k]['geometry_' + str(i)] == "Три-поверхность" or geometry_1_obj[k]['geometry_' + str(i)] == "Tri-surface" \
					or geometry_1_obj[k]['geometry_' + str(i)] == "Распределенная три-поверхность" or geometry_1_obj[k]['geometry_' + str(i)] == "Distributed tri-surface" \
					or geometry_1_obj[k]['geometry_' + str(i)] == "Закрытая три-поверхность" or geometry_1_obj[k]['geometry_' + str(i)] == "Closed tri-surface":
